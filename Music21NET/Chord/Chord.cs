...
public class ChordBase : NotRest {
    ...
    protected ChordBase(object notes) : base() {
...
public class Chord : ChordBase {
    ...
    public Chord(object notes) : base(notes) {
        isChord = true;
    ...
    private List<Note.Note> Initialize(object notes) {
        return notes switch {
            ...
            IEnumerable<Pitch.Pitch> pitchNotes => pitchNotes.Select(p => new Note.Note(p)).ToList(),
    ...
    public string PitchedCommonName {
        get {
            ...
            if (PitchClassCardinality <= 2 || nameStr.Contains("enharmonic") || nameStr.Contains("forte class") ||
